...
( function module( base ){
	...
	*/
	moduleDefine( "convertToSequence",
		function construct( ){
			var childprocess = require( "child_process" );
			var fs = require( "fs" );
			var path = require( "path" );
			var pathDirectory = process.cwd( ) + "/convert-to-sequence/".replace( "/", path.sep );
			var javaClassFile = pathDirectory + "convertToSequence.class";
			var isJavaClassFileExist = fs.existsSync( javaClassFile );

			var convertToSequenceIndex = function convertToSequence( separator, sequence, dictionary, callback ){
				var parameters = Array.prototype.slice.apply( arguments );
				...
				var command = "java " + pathDirectory + "convertToSequence " + numbers.join( " " );	
				if( !isJavaClassFileExist ){
					command = "javac " + pathDirectory + "convertToSequence.java && " + command;
				}
			...
			base.convertToSequence = convertToSequence;
		} );
} )( base );